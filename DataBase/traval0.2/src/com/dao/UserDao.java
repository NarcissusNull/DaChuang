...
public static boolean userExit(int usd,String psd) throws SQLException {
    ...
    try{//主体部分
        conn = DBAconnect.getConnection();
        if (conn != null) {
            String sql = "select password from User Where userid="+usd;//sql语句进行
            pstmt = conn.prepareStatement(sql);//编译预处理
            rs = pstmt.executeQuery();//sql执行

            while (rs.next()) {//当next指针不为空，即线性表未空
                list.add(rs.getString(1));//向线性表中添加string
               // password=rs.getString(1);1
               // if(password==psd)
                 //   flag=true;
            }
